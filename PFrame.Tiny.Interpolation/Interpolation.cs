...
///  @name Unity.Tiny
/// </summary>
namespace PFrame.Tiny.Interpolation
{
    ...
    public struct MinMaxCurve
    {
        ...
        public Entity curveMax;
        public Entity curveMin;

        public override string ToString()
        {
            return string.Format("[MMC: {0},{1},{2}]", (MinMaxCurveMode)mode, constantMax, constantMin);
        }
    }
    ...
    public static class InterpolationService
    {
        ...
        public static float3 EvaluateMinMaxCurve3(EntityManager mgr, float time, MinMaxCurve curveX, MinMaxCurve curveY, MinMaxCurve curveZ, float randomFactor = 1f)
        {
            var x = EvaluateMinMaxCurve(mgr, time, curveX, randomFactor);
            var y = EvaluateMinMaxCurve(mgr, time, curveY, randomFactor);
            var z = EvaluateMinMaxCurve(mgr, time, curveZ, randomFactor);
            return new float3(x, y, z);
        }

        public static float EvaluateMinMaxCurve(EntityManager mgr, float time, MinMaxCurve curve, float randomFactor = 1f)
        {
            var mode = curve.mode;
            if (mode == (int)MinMaxCurveMode.Constant)
                return curve.constantMax;
            if (mode == (int)MinMaxCurveMode.TwoConstants)
                return math.lerp(curve.constantMin, curve.constantMax, randomFactor);
            //return m_rand.NextFloat(curve.constantMin, curve.constantMax);
            if (mode == (int)MinMaxCurveMode.Curve)
                return EvaluateBezierCurveFloat(mgr, time, curve.curveMax);
            if (mode == (int)MinMaxCurveMode.TwoCurves)
            {
                var min = EvaluateBezierCurveFloat(mgr, time, curve.curveMin);
                var max = EvaluateBezierCurveFloat(mgr, time, curve.curveMax);
                return math.lerp(min, max, randomFactor);
                //return m_rand.NextFloat(min, max);
            }
